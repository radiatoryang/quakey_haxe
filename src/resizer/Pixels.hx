...
// from https://github.com/azrafe7/hxPixels
// under MIT License
...
abstract Pixels(PixelsData)
{
	...
	public function fillRect(x:Int, y:Int, width:Int, height:Int, value:Int):Void
	{
		...
		for (x in 0...width)
			stridePixels.setPixel32(x, 0, value);
		for (y in 0...height)
		{
	...
	#if heaps
	@:from static public function fromHeapsPixels(image:hxd.Pixels)
	{
		var pixels = new Pixels(image.width, image.height, false);
		pixels.format = switch (image.format) {
			case BGRA: PixelFormat.BGRA;
			case ARGB: PixelFormat.ARGB;
			case RGBA: PixelFormat.RGBA;
			default: PixelFormat.ARGB;
		}
		pixels.bytes = image.bytes;
...
private class PixelsData
{
	...
	public function new(width:Int, height:Int, alloc:Bool = true, format:PixelFormat = null)
	{
		...
		if (alloc)
			bytes = Bytes.alloc(this.count << 2);
...
class PixelFormat
{
	...
	{
		return (((px >> (8 * (Pixels.CHANNEL_MASK - fromFormat.A))) & 0xFF) << (8 * (Pixels.CHANNEL_MASK - toFormat.A))) | (((px >> (8 * (Pixels.CHANNEL_MASK
			- fromFormat.R))) & 0xFF) << (8 * (Pixels.CHANNEL_MASK - toFormat.R))) | (((px >> (8 * (Pixels.CHANNEL_MASK - fromFormat.G))) & 0xFF) << (8 * (Pixels.CHANNEL_MASK
				- toFormat.G))) | (((px >> (8 * (Pixels.CHANNEL_MASK - fromFormat.B))) & 0xFF) << (8 * (Pixels.CHANNEL_MASK - toFormat.B)));
	}
...
abstract Pixel(Int) from Int to Int
{
	...
	inline public function multiplyAlpha():Pixel
	{
		...
		return (this & 0xFF000000) | (iclamp(iround(fA * (this : Pixel).R)) << 16) | (iclamp(iround(fA * (this : Pixel)
			.G)) << 8) | (iclamp(iround(fA * (this : Pixel).B)));
	}
	...
	inline public function unmultiplyAlpha():Pixel
	{
		...
		return (this & 0xFF000000) | (iclamp(iround(inv_fA * (this : Pixel).R)) << 16) | (iclamp(iround(inv_fA * (this : Pixel)
			.G)) << 8) | (iclamp(iround(inv_fA * (this : Pixel).B)));
	}
